...
**實驗名稱**：V5 方法，獨立生成 Pseudo-labels（Single-Stage）  
**Run ID**：01  
...
完全沿用競賽最高分方案 [v5 (smileakklpl)](https://github.com/smileakklpl/Positive-Negative-Text-Polarity-Classification-with-NLP/blob/main/src/version5.py) 的設計，  
但解除對**外部 pseudo-label 檔案**的依賴——改由實驗自身的 Phase 0 獨立生成偽標籤。

**V5 原始方案特色**：
- Pseudo-labeling（test set 加入訓練）
...
**Exp11 與 V5 唯一差異**：Pseudo-label 來源從「外部 CSV」改為「Phase 0 自行訓練後預測」。
...
| 項目 | V5 | Exp11-01 |
|------|-----|---------|
| Pseudo-label 來源 | 外部 CSV（前版本輸出） | Phase 0 自行訓練生成 |
| 訓練流程 | 單階段（直接讀入偽標籤） | Phase 0 → Phase 1 |
| 偽標籤數量 | 取決於外部檔案 | 11,000（test set 全量） |
| 其餘所有設計 | — | 完全相同 ✓ |
| Val F1 | 0.8715 | **0.8810** |
| Val Acc | 0.8695 | **0.8805** |
...
```bash
# 訓練（Phase 0 → Phase 1）
python exp/exp11_v5_single_stage/train.py

# 使用 run01 重新預測
python exp/exp11_v5_single_stage/predict.py --run 01
```
