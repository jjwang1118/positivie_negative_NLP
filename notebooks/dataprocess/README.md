...
本目錄收錄 10 篇針對 **NLP 資料處理**（文本增強、預處理、偽標籤、雜訊處理）的學術論文筆記，用於支援二元情緒分類任務（Positive=1 / Negative=0）。
...
| 11 | [EDA_Note.md](EDA_Note.md) | EDA: Easy Data Augmentation Techniques for Boosting Performance on Text Classification Tasks | 2019 | 同義詞替換 / 隨機插入 / 交換 / 刪除（EMNLP 2019） |

> 注意：清單包含第 11 篇 EDA 作為經典基礎論文（共 11 篇筆記，前 10 篇為主要推薦）。
...
動態增強（訓練時）:
  ...
  ↓
半監督利用未標籤資料:
  SCR (LLM 語義一致性) → RobustSentimentAug (偽標籤)
  ↓
嵌入空間增強:
  MAGE (LiDA + 注意力引導)
```
...
| 增強資料品質控制 | RankAug |
| 半監督 / 偽標籤 | SCR, RobustSentimentAug |
