"""
exp/exp07_cluster_pseudo_siebert/predict.py
============================================
Ensemble prediction for exp07 (cluster pseudo-labeling, siebert encoder).

Loads best.pt from each fold, averages softmax probabilities, and writes
predictions.csv to the run directory.

Note: inference uses batched forward passes to avoid OOM with RoBERTa-large.

Usage
-----
    python exp/exp07_cluster_pseudo_siebert/predict.py            # latest run
    python exp/exp07_cluster_pseudo_siebert/predict.py --run 01   # specific run
"""
...
def batched_predict(
    ...
    """Run batched inference, return softmax probabilities [N, 2]."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="exp07 ensemble predictor")
