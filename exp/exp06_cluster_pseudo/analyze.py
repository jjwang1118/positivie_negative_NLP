"""
exp/exp06_cluster_pseudo/analyze.py
=====================================
Step 0 (optional): visualize the SBERT embedding space and evaluate
cluster purity BEFORE running train.py.

Outputs (saved to results/figures/exp06/)
-----------------------------------------
...
def encode_texts(
    ...
    """Mean-pooled SBERT embeddings → numpy [N, 384]."""
    encoder.eval()
    vecs = []
...
def cluster_purity_stats(
    ...
    Returns
    -------
    purities       : purity value for every cluster that has ≥1 labeled sample
    cluster_label  : {cluster_id → majority_label} for qualifying clusters
    """
...
def main() -> None:
    ...
    ax.set_title(f"t-SNE of SBERT Embeddings  (n={n_tsne})")
