"""
exp/exp06_cluster_pseudo/train.py
===================================
Cluster-based pseudo-labeling + supervised fine-tuning.

Pipeline
--------
1. Encode all 13k texts (2k labeled + 11k unlabeled) with frozen SBERT
   using mask-aware mean pooling → [13k × 384] embeddings.
...
Note: K-means is run once before the CV loop to avoid redundant computation.
Because K-means is fully unsupervised (labels are never used), running it on
all data (including val split embeddings) introduces no information leakage.
...
# ---------------------------------------------------------------------------
# SBERT encoding (mean pooling, frozen)
# ---------------------------------------------------------------------------
...
def encode_texts(
    ...
    """Return mask-aware mean-pooled SBERT embeddings as numpy [N, 384]."""
    encoder.eval()
    vecs = []
...
def build_pseudo_labels(
    ...
    """
    Vote cluster labels from labeled train split only, then assign
    pseudo-labels to unlabeled texts in qualifying clusters.

    Returns
    -------
    pseudo_texts  : list of unlabeled texts that received a pseudo-label
    pseudo_labels : corresponding integer labels (0 or 1)
    """
    labels_arr = np.array(all_labeled_labels)

    # Vote majority label per cluster (using train split only)
...
def main() -> None:
    ...
    # -----------------------------------------------------------------------
    # Encode all texts with frozen SBERT (once, shared across folds)
    # -----------------------------------------------------------------------
    ...
    print("\nEncoding all texts with frozen SBERT...")
    embeddings = encode_texts(all_texts, encoder, tokenizer, device,
                              max_length=tr["max_length"])
    print(f"Embeddings shape: {embeddings.shape}")

    labeled_embeddings   = embeddings[:n_labeled]
    unlabeled_embeddings = embeddings[n_labeled:]
