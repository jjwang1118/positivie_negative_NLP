"""
exp/exp02_shap_retrain/predict.py
===================================
Ensemble prediction using the 3-fold models trained in exp02.

Test samples are encoded with the ORIGINAL full text (no key-string filtering).
The model trained on filtered inputs still generalises to full text because
the attention mechanism focuses on relevant tokens at inference time.

Ensemble strategy: average softmax probabilities across all fold models,
then argmax to obtain the final label.

Output: results/exp02/{run_id}/predictions.csv
Format: row_id,LABEL

Usage
-----
    python exp/exp02_shap_retrain/predict.py            # latest run
    python exp/exp02_shap_retrain/predict.py --run 01
"""
...
def load_test_csv(path: Path) -> tuple[list[str], list[str]]:
    """Load test.csv; returns (row_ids, texts)."""
    row_ids, texts = [], []
    with open(path, encoding="utf-8") as f:
