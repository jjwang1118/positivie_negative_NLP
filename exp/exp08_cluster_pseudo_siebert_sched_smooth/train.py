"""
exp/exp08_cluster_pseudo_siebert_sched_smooth/train.py
=======================================================
Exp07 + Cosine LR scheduler with linear warmup + Label smoothing.

Changes vs exp07
----------------
1. CrossEntropyLoss(label_smoothing=cfg.training.label_smoothing)
   Softens the hard pseudo-label targets to reduce overconfidence.
2. get_cosine_schedule_with_warmup (per fold)
   LR linearly ramps from 0 → peak over warmup_ratio * total_steps,
   then decays via cosine to ~0 at the final step.
3. train_log.csv now includes a `lr` column for inspection.

Everything else (encoder, K-means, pseudo-labeling, CV) is identical to exp07.
...
def main() -> None:
    ...
    # -----------------------------------------------------------------------
    # Loss with label smoothing (shared across folds)
    # -----------------------------------------------------------------------
    ...
    for fold, (train_idx, val_idx) in enumerate(splits):
        ...
        # --- Cosine scheduler with linear warmup (per fold) ----------------
